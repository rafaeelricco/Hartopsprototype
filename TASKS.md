...
> **Status: Not started** — This section covers the native mobile app, which is not part of the current web prototype scope.
...
- [x] 1. **Expand campaign creation** — Added fields: supplier, distributors, target markets/geography, anticipated event count, linked products, objectives, and channels. Campaign data model expanded in `campaign-data.ts`. Campaign detail view shows context panel.
...
- [x] 2. **Event creation inherits from campaign** — Events pre-populate objectives, products, and venue type from parent campaign/activity. Old `event-wizard.tsx` removed; events now created via `/staff/events/create?campaign={id}`. Campaign → Activity → Event hierarchy fully implemented with `activity-data.ts`.
...
- [x] 3. **Output-first report preview** — Projected impact display merged into the objectives step (step 3) as a live sidebar. No longer buried at a separate step. Wizard reduced from 5 steps to 4.
...
- [x] 5. **Structured questionnaire system** — Questionnaire template system implemented in `questionnaire-data.ts` with default questions, campaign-specific templates, multiple question types (rating, yes-no, multiple-choice, open-text), and a `getQuestionnaireForCampaign()` helper.
...
- [x] 7. **Connect item master to campaigns/events** — `EventItem` now has `linkedProductIds` field, and Activities define product subsets. Products at campaign level flow to events.
...
- [x] 3. **Product sales counter model** — Sales Volume Tracker data module implemented in `event-data.ts` tracking units sold by SKU during events. Mapped to "Drive Sales" objective.
...
- [~] 1. **Educator roster page** — `educators-page.tsx` created with roster metrics, search, multi-filter controls (status, location, specialty), quality score display with trend indicators, and pagination. **Partial:** Page exists but is not yet linked in the ops sidebar navigation, and the backing `educator-data.ts` file is missing (imported but not created).
...
- [~] 5. **Educator scoring foundation** — Quality score display implemented in `educators-page.tsx` with color coding, trend tracking (up/down/stable), and average score calculation. **Partial:** UI displays scores but no data capture flow exists for cancellations, certifications, or on-time rate that would feed the scoring.
...
- [~] 1. **Hart-level Account Master** — `accounts-page.tsx` created with account type/status filters, contact info, events hosted counter, and pagination. **Partial:** Page exists but is not linked in ops sidebar, and `account-data.ts` is missing (imported but not created). No central account catalog data model yet.
