...
* **Event creation.** This is the most complex flow and the area where the platform needs to shine. Event creation walks through several steps: basics (name, location, date, time, products from the brand asset library, venue type, instructions, compensation), objectives (brand awareness, drive sales, competitive intelligence, product testing, etc.), a tentative event report preview, and optionally direct editing of the event mechanics. The tentative report step is important: the operator sees what a concluded event report *would* look like given their configuration, and can add or remove data points. If that's not enough control, they can directly configure events.   
