...
export function BrandAmbassadorsPage() {
  ...
  const filtered = mockBrandAmbassadors
    ...
    <TooltipProvider>
      <div className="p-6 space-y-6">
        <PageHeader
          ...
          subtitle={`Your assigned brandAmbassador roster \u2014 ${mockBrandAmbassadors.length} brandAmbassadors.`}
          className="mb-0"
        />
          ...
          <div className="flex gap-1 p-1 bg-muted rounded-lg">
            {(["All", "Active", "Inactive"] as StatusFilter[]).map((tab) => (
              ...
              >
                ...
                >
                  {tab === "All"
                    ? mockBrandAmbassadors.length
                    : mockBrandAmbassadors.filter((e) => e.status === tab).length}
                </span>
              </button>
            ))}
          </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {filtered.map((edu) => (
                <div
                  key={edu.id}
                  onClick={() => navigate(`/market-manager/brand-ambassadors/${edu.id}`)}
