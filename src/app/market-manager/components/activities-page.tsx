...
type SortOption = "date" | "status" | "brand ambassador";
...
function sortEvents(events: Activity[], sortBy: SortOption): Activity[] {
  return [...events].sort((a, b) => {
    switch (sortBy) {
      case "status":
        return STATUS_SORT_ORDER[a.status] - STATUS_SORT_ORDER[b.status];
      case "brand ambassador":
        return (a.brandAmbassadorName || "zzz").localeCompare(b.brandAmbassadorName || "zzz");
      case "date":
      default: {
...
function matchesFilter(event: Activity, tab: FilterTab): boolean {
...
/* --- BrandAmbassador column with assignment status indicator --- */

function BrandAmbassadorCell({ event }: { event: Activity }) {
  if (!event.brandAmbassadorName) {
    return (
      <span
  ...
  const extraCount =
    event.assignedBrandAmbassadors && event.assignedBrandAmbassadors.length > 1
      ? event.assignedBrandAmbassadors.length - 1
      : 0;
  ...
  return (
    ...
    >
      <User className="w-3.5 h-3.5 flex-shrink-0" />
      <span className="truncate">{event.brandAmbassadorName}</span>
      {extraCount > 0 && (
        <span
      ...
      {event.status === "Pending" && (
        ...
        >
          ...
          Pending
          {event.assignedBrandAmbassadors && event.assignedBrandAmbassadors.length > 0 && (
            <span className="ml-0.5">
              (
              {
                event.assignedBrandAmbassadors.filter(
                  (ae) =>
                    ae.assignmentStatus !== "Pending" &&
                    ae.assignmentStatus !== "Withdrawn",
                ).length
              }
              /{event.assignedBrandAmbassadors.length})
            </span>
          )}
        </span>
      )}
...
function EventNameCell({ event }: { event: Activity }) {
  return (
  ...
  const [editedNotesMap, setEditedNotesMap] = useState<Record<string, string>>(
    () => {
      ...
      pendingEvents.forEach((e) => {
        if (e.brandAmbassadorNotesFinal) initial[e.id] = e.brandAmbassadorNotesFinal;
      });
      return initial;
    },
  );
              ...
              >
                ...
                {isExpanded && event.finalStats && (
                  <div className="px-5 pb-5 space-y-4 border-t border-border">
                    ...
                    {/* BrandAmbassador notes — editable before finalize */}
                    ...
                    {showConfirm === event.id ? (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            >
                              ...
                              brandAmbassador&apos;s editing window.
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <Link
                          to={`/market-manager/activities/${event.id}`}
...
export function ActivitiesPage() {
    ...
    mockEvents
      ...
      .filter(
        (e) =>
          ...
          (e.brandAmbassadorName || "").toLowerCase().includes(search.toLowerCase()),
      )
  ...
  const cardTitle =
    activeFilter === "All" ? "All Activities" : `${activeFilter} Events`;
  ...
  const cycleSortOption = () => {
    const options: SortOption[] = ["date", "status", "brand ambassador"];
              ...
              <div className="divide-y divide-border">
                {filteredEvents.map((event) => (
                  ...
                  >
                    ...
                    {/* BrandAmbassador */}
                    <BrandAmbassadorCell event={event} />
                    {/* Status */}
                    <div className="flex items-center gap-2">
                      <span
