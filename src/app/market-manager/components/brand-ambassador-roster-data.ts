// BrandAmbassador Roster mock data — scoped to authenticated manager's assigned brandAmbassadors
...
function generateAvailability(seed: number): BrandAmbassador["availability"] {
  const start = new Date("2026-03-18");
  const result: BrandAmbassador["availability"] = [];
...
export const mockBrandAmbassadors: BrandAmbassador[] = [
  {
    id: "edu-1",
    name: "Ana Martinez",
...
export function getBrandAmbassadorById(id: string): BrandAmbassador | undefined {
  return mockBrandAmbassadors.find((e) => e.id === id);
}
