// BrandAmbassador Scoring & Metrics utilities
 ...
 */
export function computeQualityScore(brandAmbassador: BrandAmbassador): number {
  const retailSalesNorm = Math.min(brandAmbassador.retailSalesAvg / 20, 1); // 20 units/event = 100%
  const checkInNorm = brandAmbassador.checkInScore / 100;
  const completionNorm = brandAmbassador.eventCompletionAvg / 100;
  const surveyNorm = brandAmbassador.retailerSurveyScore / 5;
  const cancellationNorm = brandAmbassador.cancellationRating / 100;
...
export interface MetricDefinition {
  ...
  trendKey: keyof BrandAmbassador["trends"];
}
