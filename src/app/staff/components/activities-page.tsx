...
// Includes inline brandAmbassador quick-assign from the list view.
// =============================================================================
...
import { type Activity, type AssignedBrandAmbassadorRecord } from "./activity-data";
import { INITIAL_REGIONS } from "./settings-data";
import { MOCK_BRAND_AMBASSADORS } from "../../ops/components/brand-ambassador-data";
...
const STATUS_FILTERS: (Activity["status"] | "all")[] = [
  "all",
  "draft",
  "scheduled",
  "active",
...
/** Format brandAmbassador names for display: "Maria S. +2" */
function getAssignmentLabel(brandAmbassadors: AssignedBrandAmbassadorRecord[]): string {
  ...
  if (brandAmbassadors.length === 1) return short;
  return `${short} +${brandAmbassadors.length - 1}`;
}
...
export function ActivitiesPage() {
  ...
  const getAssignments = useCallback(
    (event: Activity): AssignedBrandAmbassadorRecord[] => {
      const local = localAssignments[event.id];
      if (local) return local;
      return event.assignedBrandAmbassadors ?? [];
    },
    [localAssignments],
  );
  ...
  const handleOpenAssignment = useCallback(
    (activityId: string) => {
      ...
      const current =
        localAssignments[activityId] ?? event.assignedBrandAmbassadors ?? [];
  ...
  const handleConfirmAssignments = useCallback(() => {
    ...
    const existing =
      localAssignments[activeEventId] ?? event.assignedBrandAmbassadors ?? [];
    const existingMap = new Map(existing.map((e) => [e.brandAmbassadorId, e]));
    ...
    for (const eduId of draftSelectedIds) {
      ...
      if (prev) {
        ...
        const brandAmbassador = MOCK_BRAND_AMBASSADORS.find((e) => e.id === eduId);
        if (brandAmbassador) {
          newAssignments.push({
            brandAmbassadorId: brandAmbassador.id,
            name: brandAmbassador.name,
            status: "Pending",
          });
        }
      }
  ...
  const filtered = useMemo(() => {
    ...
    if (assignmentFilter !== "all") {
      result = result.filter((e) => {
        const brandAmbassadors = getAssignments(e);
        if (assignmentFilter === "unassigned") return brandAmbassadors.length === 0;
        if (assignmentFilter === "assigned") return brandAmbassadors.length > 0;
        if (assignmentFilter === "pending")
          return brandAmbassadors.some((ed) => ed.status === "Pending");
        return true;
      });
    }
    if (regionFilter !== "all") {
          ...
          <DialogHeader className="px-5 pt-5 pb-3 shrink-0 border-b border-[#E2E8F0]">
            ...
            <div className="relative mt-3">
              ...
              <Input
                ...
                placeholder="Search brand ambassadors by name, location, or specialty..."
                className="pl-9 h-9"
              />
            </div>
          </DialogHeader>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Checkbox
                          id={`staff-edu-${brandAmbassador.id}`}
                          checked={isSelected}
                          className="mt-0.5"
                        ...
                        <div className="flex items-center justify-between flex-1 min-w-0">
                          <div className="space-y-0.5 flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <label
                                htmlFor={`staff-edu-${brandAmbassador.id}`}
...
function EventTableView({
  ...
  getAssignments: (event: Activity) => AssignedBrandAmbassadorRecord[];
  onOpenAssignment: (activityId: string) => void;
}) {
  if (events.length === 0) {
    return (
        ...
        >
          ...
          <tbody>
            {events.map((event) => {
              ...
              const hasPending = brandAmbassadors.some((e) => e.status === "Pending");
...
function EventCalendarView({
  ...
  eventsByDate: Map<string, Activity[]>;
