...
// Accessible from /staff/activities/create
// =============================================================================
...
const STEPS = [
  ...
  { num: 2, label: "Activity Basics", icon: FileText },
...
export function CreateActivityPage() {
  ...
  const preselectedCampaign = campaignIdParam
    ...
    : undefined;
  const preselectedTemplate = templateIdParam
  ...
  const backUrl = campaignIdParam
    ...
    : "/staff/activities";
  const backLabel = campaignIdParam ? "Back to Campaign" : "Back to Activities";
  ...
  const [data, setData] = useState<WizardData>(() => {
    if (preselectedCampaign) {
      ...
      return {
        ...
        ...(preselectedTemplate?.venueType
          ? { venueType: preselectedTemplate.venueType }
          : {}),
      };
    }
    return INITIAL_DATA;
  ...
  const [showBrandAmbassador, setShowBrandAmbassador] = useState(false);
  ...
  function validateStep4(): boolean {
    // Billing step — soft validation. BA roster is optional (per Leah,
    // May-26): users may not know assignments at creation. Only validate that
    // any BA row with an explicit rate override has a reason picked.
    let msg: string | null = null;
    if (billing.activityType === "event") {
      const missingReason = billing.bas.some(
        (b) => b.brandAmbassadorId && b.overrideRate > 0 && !b.overrideReason,
      );
      if (missingReason) {
        msg = "Pick a reason for every rate override.";
      }
    }
    setBillingError(msg);
    return msg === null;
  ...
  function handleSubmit() {
    ...
    const event = createActivity({
      ...
      ...(preselectedTemplate ? { templateId: preselectedTemplate.id } : {}),
      name: data.name.trim(),
      ...
      venueType: data.venueType as Activity["venueType"],
    ...
    <div className="min-h-full flex flex-col font-[Inter]">
      ...
      <div className="px-6 pt-5 pb-0">
        ...
        <p style={{ fontSize: "0.875rem", color: "#94A3B8" }} className="mb-5">
          Walk through each step to set up your activity.
        </p>
      </div>
      ...
      {preselectedTemplate && (
        ...
        >
          <Target size={14} />
          Creating activity from template:{" "}
          <strong>{preselectedTemplate.name}</strong>
        </div>
      )}
      ...
      <div className="flex-1 px-6 pb-6 overflow-y-auto">
        {step === 1 && (
          <StepCampaign
            ...
            getActivitiesForCampaign={getActivitiesForCampaign}
          />
        )}
        {step === 2 && (
          <StepBasics
            ...
            {data.objectives.length > 0 && (
              <>
                ...
                <ProjectedImpactSection
                  ...
                  showBrandAmbassador={showBrandAmbassador}
                  onToggleBrandAmbassador={() => setShowBrandAmbassador((p) => !p)}
                />
              </>
            )}
          </div>
      ...
      <div className="sticky bottom-0 bg-white border-t border-[#E2E8F0] px-6 py-4 flex items-center justify-between">
        ...
        {step < 6 ? (
          ...
          >
            <Check size={15} />
            Create {billing.activityType === "survey" ? "Survey" : "Event"}
          </Button>
        )}
      </div>
...
function StepCampaign({
  ...
  getActivitiesForCampaign: (id: string) => Activity[];
}) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {filtered.map((campaign) => {
          ...
          const activityCount = getActivitiesForCampaign(campaign.id).length;
          return (
            <Button
              key={campaign.id}
              variant="ghost"
...
function ProjectedImpactSection({
  ...
  showBrandAmbassador: boolean;
  onToggleBrandAmbassador: () => void;
}) {
    ...
    <div style={{ animation: "fadeInUp 0.3s ease-out" }}>
      <div className="flex items-center justify-between mb-1">
        ...
        <Button
          variant="ghost"
          type="button"
          onClick={onToggleBrandAmbassador}
        ...
        >
          <Smartphone size={13} />
          BrandAmbassador View
        </Button>
      </div>
      <p style={{ fontSize: "0.8125rem", color: "#94A3B8" }} className="mb-4">
        {showBrandAmbassador
          ? "Preview of what the brand ambassador will see on their mobile device."
      ...
      {!showBrandAmbassador ? (
        ...
        /* BrandAmbassador mobile preview — centered in full width */
