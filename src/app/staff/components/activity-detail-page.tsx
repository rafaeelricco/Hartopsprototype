...
function getPhase(status: Activity["status"]): LifecyclePhase {
...
> = {
  1: {
    ...
    description: "Activity details can be modified before the event starts.",
    icon: Edit3,
    color: "#1D4ED8",
    bg: "#EFF6FF",
  },
  2: {
    ...
    description: "Real-time data is flowing in from field brand ambassadors.",
    icon: Radio,
    color: "#0F766E",
    bg: "#ECFDF5",
  },
...
export function ActivityDetailPage() {
  ...
  const { getActivity, getCampaign, updateActivityStatus } = useCampaignContext();

  const event = getActivity(activityId ?? "");
  ...
  function executeTransition() {
    ...
    updateActivityStatus(event.id, confirmAction.status);
        ...
        ? {
            ...
            onClick: () =>
              requestTransition(
                "active",
                "Start this event?",
                "Starting the event transitions it to the Live Data Feed phase. Field brand ambassadors will begin submitting data.",
...
function EditableField({
  ...
  const { updateActivityFields } = useCampaignContext();
  ...
  function save() {
    if (draft.trim() && draft !== value) {
      updateActivityFields(activityId, { [field]: draft.trim() } as any);
    }
    setEditing(false);
  }
      ...
      <div className="lg:col-span-2 bg-white rounded-xl border border-[#E2E8F0] overflow-hidden">
            ...
            >
              ...
              >
                {item.type === "photo" ? (
                  ...
                  <ActivityIcon size={14} style={{ color: "#0F766E" }} />
                )}
              </div>
              <div className="flex-1 min-w-0">
        ...
        <div className="px-5 py-3 border-t border-[#E2E8F0] bg-[#FAFBFC] flex items-center gap-2">
          ...
          <p style={{ fontSize: "0.6875rem", color: "#94A3B8" }}>
            Simulating near real-time feed from field brandAmbassadors ·{" "}
...
function ProductsSamplesCard({ event }: { event: Activity }) {
