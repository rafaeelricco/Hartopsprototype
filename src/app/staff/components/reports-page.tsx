...
type SortKey = "campaignName" | "events" | "samples" | "consumerReach" | "totalSales" | "conversionRate" | "avgEngagement";
...
export function ReportsPage() {
  ...
  const comparisonData = useMemo(() => {
    let rows = CAMPAIGN_METRICS.filter((c) => selectedCampaigns.has(c.campaignId));
    rows = [...rows].sort((a, b) => {
      const av = a[sortKey];
      const bv = b[sortKey];
  ...
  // ── Gallery (change #4) ─────────────────────────────────────────────────
  const [galleryCampaignFilter, setGalleryCampaignFilter] = useState<string>("all");
  const [galleryEventFilter, setGalleryEventFilter] = useState<string>("all");
  const [gallerySortDir, setGallerySortDir] = useState<GallerySortDir>("newest");
  ...
  const galleryEventOptions = useMemo(() => {
    ...
    return events.filter((e) =>
      PROOF_PHOTOS.some((p) => p.eventId === e.eventId && p.campaignId === galleryCampaignFilter)
    );
    ...
    <div className="p-6 font-[Inter]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p style={{ fontSize: "0.875rem", color: "#94A3B8" }}>
            Centralized performance tracking across events and campaigns.
          </p>
        </div>
              ...
              <>
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl border border-[#E2E8F0] shadow-lg z-50 overflow-hidden">
                  <div className="px-4 py-2.5 border-b border-[#F1F5F9]">
                    <p style={{ fontSize: "0.6875rem", color: "#94A3B8" }}>SELECT FORMAT</p>
                  </div>
                  <button
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "#7D152D0F" }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: "0.6875rem", color: "#94A3B8" }}>Branded, client-facing</p>
                    </div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 w-full px-4 py-3 hover:bg-[#F8FAFC] transition-colors text-left disabled:opacity-60"
                  >
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "#0F766E0F" }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: "0.6875rem", color: "#94A3B8" }}>Raw data for analysts</p>
                    </div>
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-2 mb-3">
          <span style={{ fontSize: "0.6875rem", color: "#94A3B8", letterSpacing: "0.04em" }}>
            QUICK STATS
          </span>
        ...
        <div className="grid grid-cols-3 gap-3">
          <SecondaryStatCard icon={<CalendarDays size={14} />} label="Events" value={stats.events.toString()} color="#D97706" />
          <SecondaryStatCard icon={<Hash size={14} />} label="Social Mentions" value={formatNum(stats.socialMentions)} color="#7C3AED" />
          <SecondaryStatCard icon={<Camera size={14} />} label="Photos" value={stats.photoCount.toString()} color="#059669" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#E2E8F0] p-5 mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span style={{ fontSize: "0.9375rem", color: "#0F172A" }}>Performance Trend</span>
          <span style={{ fontSize: "0.75rem", color: "#94A3B8" }} className="ml-1">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={trendData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
            ...
            <YAxis tick={{ fill: "#94A3B8", fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={formatNum} />
            <Tooltip
              contentStyle={{ borderRadius: 10, border: "1px solid #E2E8F0", fontSize: 12, boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}
              formatter={(value: number, name: string) => {
                if (name === "sales") return [fmtCurrency(value), "Sales"];
                if (name === "reach") return [formatNum(value), "Consumer Reach"];
            ...
            <Legend
              ...
              formatter={(value: string) => {
                const labels: Record<string, string> = { samples: "Samples", reach: "Consumer Reach", sales: "Sales" };
                return labels[value] ?? value;
              }}
            />
            <Area type="monotone" dataKey="samples" stroke="#7D152D" strokeWidth={2} fill="url(#gradSamples)" />
            <Area type="monotone" dataKey="reach" stroke="#1D4ED8" strokeWidth={2} fill="url(#gradReach)" />
            <Area type="monotone" dataKey="sales" stroke="#0F766E" strokeWidth={2} fill="url(#gradSales)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
        ...
        <div className="px-5 py-4 border-b border-[#E2E8F0]">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
            <div className="flex items-center gap-2">
              ...
              <span style={{ fontSize: "0.9375rem", color: "#0F172A" }}>Campaign Comparison</span>
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg transition-all flex items-center gap-1 ${
                    selected ? "text-white" : "text-[#64748B] bg-[#F1F5F9] hover:bg-[#E2E8F0]"
                  }`}
                  style={selected ? { background: "#7D152D", fontSize: "0.6875rem" } : { fontSize: "0.6875rem" }}
                  title={c.campaignName}
                >
                  {selected && <CheckCircle2 size={10} />}
                  {c.campaignName.length > 20 ? c.campaignName.slice(0, 18) + "..." : c.campaignName}
                </button>
              );
            })}
          </div>
        ...
        {selectedCampaigns.size === 0 ? (
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[860px]" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
                <thead>
                    ...
                    {([
                      ...
                      >
                        <span
                          ...
                          style={{ fontSize: "0.6875rem", color: sortKey === col.key ? "#7D152D" : "#94A3B8", letterSpacing: "0.04em" }}
                        >
                          {col.label}
                          <ArrowUpDown size={10} className="opacity-40 group-hover:opacity-100 transition-opacity" />
                          {sortKey === col.key && (
                            sortDir === "desc"
                              ? <ChevronDown size={9} style={{ color: "#7D152D" }} />
                              : <ChevronUp size={9} style={{ color: "#7D152D" }} />
                          )}
                        </span>
                      </th>
                    ))}
                    <th className="px-4 py-3 text-left" style={{ fontSize: "0.6875rem", color: "#94A3B8", letterSpacing: "0.04em" }}>
                      Social
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {comparisonData.map((row) => {
                    ...
                    return (
                      <tr key={row.campaignId} className="border-b border-[#F8FAFC] last:border-0 hover:bg-[#FAFBFC] transition-colors">
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="flex-shrink-0 w-1.5 h-1.5 rounded-full" style={{ background: dotColor }} title={row.status} />
                          </div>
                        </td>
                        <td className="px-4 py-3.5" style={{ fontSize: "0.8125rem", color: "#0F172A" }}>{row.events}</td>
                        <td className="px-4 py-3.5" style={{ fontSize: "0.8125rem", color: "#0F172A" }}>{row.samples.toLocaleString()}</td>
                        <td className="px-4 py-3.5" style={{ fontSize: "0.8125rem", color: "#0F172A" }}>{row.consumerReach.toLocaleString()}</td>
                        <td className="px-4 py-3.5" style={{ fontSize: "0.8125rem", color: "#0F172A" }}>{fmtCurrency(row.totalSales)}</td>
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2">
                            <div className="w-14 h-1.5 rounded-full bg-[#E2E8F0] overflow-hidden">
                              <div className="h-full rounded-full" style={{ width: `${row.conversionRate}%`, background: "#0F766E" }} />
                            </div>
                            <span style={{ fontSize: "0.75rem", color: "#0F172A" }}>{row.conversionRate}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3.5">
                          <span
                            ...
                            style={{
                              fontSize: "0.75rem",
                              background: row.avgEngagement >= 8.5 ? "#ECFDF5" : row.avgEngagement >= 7 ? "#EFF6FF" : "#FEF3C7",
                              color: row.avgEngagement >= 8.5 ? "#0F766E" : row.avgEngagement >= 7 ? "#1D4ED8" : "#92400E",
                            }}
                          >
                            {row.avgEngagement}/10
                          </span>
                        </td>
                        <td className="px-4 py-3.5" style={{ fontSize: "0.75rem", color: "#64748B" }}>{row.socialMentions.toLocaleString()}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="px-5 py-3 border-t border-[#E2E8F0] bg-[#FAFBFC] flex items-center gap-6 flex-wrap">
              <span style={{ fontSize: "0.6875rem", color: "#94A3B8", letterSpacing: "0.04em" }}>TOTALS</span>
              <span style={{ fontSize: "0.75rem", color: "#0F172A" }}>{comparisonData.reduce((s, c) => s + c.events, 0)} events</span>
              <span style={{ fontSize: "0.75rem", color: "#0F172A" }}>{comparisonData.reduce((s, c) => s + c.samples, 0).toLocaleString()} samples</span>
              <span style={{ fontSize: "0.75rem", color: "#0F172A" }}>{fmtCurrency(comparisonData.reduce((s, c) => s + c.totalSales, 0))} sales</span>
              <span style={{ fontSize: "0.75rem", color: "#0F172A" }}>{comparisonData.reduce((s, c) => s + c.consumerReach, 0).toLocaleString()} reach</span>
            </div>
          </>
        )}
      </div>
      ...
      {selectedCampaigns.size > 0 && (
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-5 mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span style={{ fontSize: "0.9375rem", color: "#0F172A" }}>Sales by Campaign</span>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={comparisonData.map((c) => ({
                name: c.campaignName.length > 14 ? c.campaignName.slice(0, 12) + "..." : c.campaignName,
                sales: c.totalSales,
                samples: c.samples,
              }))}
            ...
            >
              ...
              <Tooltip
                contentStyle={{ borderRadius: 10, border: "1px solid #E2E8F0", fontSize: 12, boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}
                formatter={(value: number, name: string) => {
                  if (name === "sales") return [fmtCurrency(value), "Total Sales"];
                  return [formatNum(value), "Samples"];
                }}
              />
              <Bar dataKey="sales" fill="#7D152D" radius={[4, 4, 0, 0]} barSize={28} />
              <Bar dataKey="samples" fill="#0F766E" radius={[4, 4, 0, 0]} barSize={28} opacity={0.7} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-[#E2E8F0] overflow-hidden mb-2">
        <div className="px-5 py-4 border-b border-[#E2E8F0]">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
            <div className="flex items-center gap-2">
              ...
              <span style={{ fontSize: "0.9375rem", color: "#0F172A" }}>Photo Proof Gallery</span>
              <span style={{ fontSize: "0.75rem", color: "#94A3B8" }}>({galleryPhotos.length} photos)</span>
            </div>
            <div className="flex items-center gap-2">
              {/* Date sort toggle */}
              <button
                onClick={() => setGallerySortDir((d) => (d === "newest" ? "oldest" : "newest"))}
              ...
              {galleryEventOptions.length > 1 && (
                <div className="relative">
                  ...
                  <ListFilter size={11} className="absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: "#94A3B8" }} />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg transition-colors ${
                galleryCampaignFilter === "all" ? "text-white" : "text-[#64748B] bg-[#F1F5F9] hover:bg-[#E2E8F0]"
              }`}
              style={galleryCampaignFilter === "all" ? { background: "#7D152D", fontSize: "0.75rem" } : { fontSize: "0.75rem" }}
            >
              All
            </button>
            {galleryCampaigns.map(([id, name]) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-colors ${
                  galleryCampaignFilter === id ? "text-white" : "text-[#64748B] bg-[#F1F5F9] hover:bg-[#E2E8F0]"
                }`}
                style={galleryCampaignFilter === id ? { background: "#7D152D", fontSize: "0.75rem" } : { fontSize: "0.75rem" }}
              >
        ...
        {galleryPhotos.length === 0 ? (
          <div className="px-5 py-12 text-center">
            <p style={{ fontSize: "0.875rem", color: "#94A3B8" }}>No photos match the current filters.</p>
          </div>
        ) : (
            ...
            {galleryPhotos.map((photo, idx) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-end p-3">
                  <p className="text-white truncate" style={{ fontSize: "0.75rem" }}>{photo.eventName}</p>
                  <p className="text-white/70 truncate" style={{ fontSize: "0.625rem" }}>{photo.location}</p>
                  <p className="text-white/50" style={{ fontSize: "0.5625rem" }}>
                    {new Date(photo.date + "T12:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                  </p>
                </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] p-5 flex items-start gap-4">
      ...
      <div className="min-w-0">
        <p style={{ fontSize: "0.6875rem", color: "#94A3B8" }} className="mb-1 truncate">
          {label}
        </p>
        <p style={{ fontSize: "1.375rem", color: "#0F172A", lineHeight: 1.2 }}>{value}</p>
        <span
          className="inline-flex items-center gap-0.5 mt-1"
          style={{ fontSize: "0.6875rem", color: delta >= 0 ? "#0F766E" : "#B91C1C" }}
        >
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] px-4 py-3 flex items-center gap-3">
      ...
      <div className="min-w-0">
        ...
        <p style={{ fontSize: "0.625rem", color: "#94A3B8" }} className="truncate">{label}</p>
      </div>
    </div>
  );
}
...
function Spinner() {
  return (
    <svg className="ml-auto w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none">
      <circle cx="12" cy="12" r="10" stroke="#E2E8F0" strokeWidth="3" />
      <path d="M12 2a10 10 0 0 1 10 10" stroke="#7D152D" strokeWidth="3" strokeLinecap="round" />
    </svg>
  );
}
      ...
      >
        <div className="relative">
          ...
          <div
            ...
            style={{ background: "rgba(0,0,0,0.55)", fontSize: "0.6875rem", color: "#fff" }}
          >
            {index + 1} / {total}
          </div>
        </div>
        ...
        <div className="p-5">
          ...
          <div className="flex items-center gap-3">
            <span className="px-2.5 py-1 rounded-lg" style={{ fontSize: "0.6875rem", background: "#7D152D0F", color: "#7D152D" }}>
              <Megaphone size={10} className="inline mr-1" style={{ verticalAlign: "-1px" }} />
              {photo.campaignName}
            </span>
          ...
          {/* Keyboard hint */}
          <p className="mt-3 pt-3 border-t border-[#F1F5F9]" style={{ fontSize: "0.625rem", color: "#CBD5E1" }}>
            Use <kbd className="px-1 py-0.5 rounded bg-[#F1F5F9] text-[#94A3B8]" style={{ fontSize: "0.5625rem" }}>←</kbd>{" "}
            <kbd className="px-1 py-0.5 rounded bg-[#F1F5F9] text-[#94A3B8]" style={{ fontSize: "0.5625rem" }}>→</kbd> to navigate
            {" "}&middot;{" "}
            <kbd className="px-1 py-0.5 rounded bg-[#F1F5F9] text-[#94A3B8]" style={{ fontSize: "0.5625rem" }}>Esc</kbd> to close
          </p>
        </div>
      </div>
    </div>
