...
import { CompensationPanel } from "../../market-manager/components/compensation-panel";
import { PayHistoryPanel } from "../../market-manager/components/pay-history-panel";
...
/* ------------------------------------------------------------------ */
/* Mock recent event history per brandAmbassador                              */
/* ------------------------------------------------------------------ */
...
const RECENT_EVENTS_BY_BRAND_AMBASSADOR: Record<string, RecentEvent[]> = {
  "EDU-001": [
    {
      id: "EVT-1081",
...
function getDefaultEvents(brandAmbassadorId: string): RecentEvent[] {
  return [
    {
      id: `EVT-${Math.abs(brandAmbassadorId.charCodeAt(4) * 100 + 1000)}`,
      name: "Brand Activation Activity",
      account: "Total Wine & More",
      date: "2026-03-10",
      status: "completed",
      salesUnits: 30,
...
export function BrandAmbassadorDetailPage() {
  ...
  const brandAmbassador = MOCK_BRAND_AMBASSADORS.find((e) => e.id === id);

  if (!brandAmbassador) {
    return (
      <div className="p-6 font-[Inter]">
        <button
          onClick={() => navigate("/ops/dashboard/brand-ambassadors")}
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p style={{ fontSize: "0.875rem", color: "#94A3B8" }}>
            No brand ambassador with ID &quot;{id}&quot; exists.
          </p>
        </div>
      </div>
    );
  ...
  const recentEvents =
    RECENT_EVENTS_BY_BRAND_AMBASSADOR[brandAmbassador.id] ?? getDefaultEvents(brandAmbassador.id);
    ...
    <div className="p-6 space-y-6 font-[Inter]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                >
                  {brandAmbassador.status.charAt(0).toUpperCase() +
                    brandAmbassador.status.slice(1)}
                </span>
                {brandAmbassador.qualityScore > 0 && (
                  ...
                  >
                    <Star
                      ...
                      style={{ color: "#F59E0B" }}
                    />
                    {brandAmbassador.qualityScore} Quality
                  </span>
                )}
              </div>
              <div className="flex items-center gap-4 flex-wrap">
                ...
                >
                  ...
                  {brandAmbassador.eventsCompleted}{" "}
                  {brandAmbassador.eventsCompleted === 1 ? "activity" : "activities"}{" "}
                  completed
                </span>
                <span
                  className="flex items-center gap-1.5"
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <InfoCard
          icon={MapPin}
          label="Location"
          value={`${brandAmbassador.city}, ${brandAmbassador.state}`}
        />
        ...
        <InfoCard
          ...
          label="Status"
          value={
            brandAmbassador.status.charAt(0).toUpperCase() + brandAmbassador.status.slice(1)
          }
        />
        ...
        <InfoCard
          ...
          value={brandAmbassador.eventsCompleted}
        ...
        <InfoCard
          ...
          label="Last Activity"
          value={
            brandAmbassador.lastEventDate
              ? formatDate(brandAmbassador.lastEventDate)
              : "No events yet"
          }
        />
        ...
        <InfoCard
          icon={Star}
          label="Quality Score"
          value={brandAmbassador.qualityScore > 0 ? brandAmbassador.qualityScore : "—"}
          subValue={
            brandAmbassador.qualityScore > 0
              ? `${brandAmbassador.trend === "up" ? "↑" : brandAmbassador.trend === "down" ? "↓" : "→"} Trending ${brandAmbassador.trend}`
              : "Not yet scored"
          }
        />
          ...
          <InfoCard
            ...
            value={
              <div className="flex flex-wrap gap-1.5 mt-1">
                {brandAmbassador.specialties.map((s) => (
                  <span
                    key={s}
      ...
      <CompensationPanel
        ...
        initialStandardRate={brandAmbassador.standardRate ?? 0}
        initialEffectiveDate={
          brandAmbassador.standardRateEffectiveDate ??
      ...
      {brandAmbassador.qualityScore > 0 && (
        <div className="space-y-3">
                ...
                <div className="flex-1">
                  ...
                  >
                    <span
                      ...
                      style={{
                        color:
                          brandAmbassador.trend === "up"
                            ? "#22C55E"
                            : brandAmbassador.trend === "down"
                              ? "#EF4444"
                              : "#94A3B8",
                      }}
                    >
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {/* Reliability */}
            {(() => {
              const val = Math.min(100, brandAmbassador.qualityScore + 3);
              const color =
            ...
            {/* On-Time Rate */}
            {(() => {
              const val = Math.max(70, brandAmbassador.qualityScore - 5);
              const color =
                val >= 80 ? "#22C55E" : val >= 60 ? "#F59E0B" : "#EF4444";
              return (
                <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 flex flex-col gap-1.5">
            ...
            {/* Survey Quality */}
            {(() => {
              const val = Math.min(100, brandAmbassador.qualityScore + 1);
              const color =
                val >= 80 ? "#22C55E" : val >= 60 ? "#F59E0B" : "#EF4444";
              return (
                <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 flex flex-col gap-1.5">
      ...
      >
        ...
        />
        <p style={{ fontSize: "0.8125rem", color: "#94A3B8" }}>
          This is a read-only brandAmbassador profile. Assignment management and
