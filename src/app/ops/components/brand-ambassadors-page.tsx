...
function TrendIcon({ trend }: { trend: BrandAmbassador["trend"] }) {
  switch (trend) {
    case "up":
...
const STATES = Array.from(new Set(MOCK_BRAND_AMBASSADORS.map((e) => e.state))).sort();

const ALL_SPECIALTIES = Array.from(
  new Set(MOCK_BRAND_AMBASSADORS.flatMap((e) => e.specialties)),
).sort();
...
export function BrandAmbassadorsPage() {
  ...
  const filtered = useMemo(() => {
    let result = [...MOCK_BRAND_AMBASSADORS];
  ...
  return (
    <div className="p-6 space-y-6 w-full">
      {/* Header */}
      <div>
        <h1 className="text-foreground">Brand Ambassadors</h1>
        <p
          className="text-muted-foreground mt-1"
          style={{ fontSize: "0.875rem" }}
        >
          Manage brandAmbassador roster, availability, and assignments.
        </p>
      </div>
                  ...
                  <tr>
                    <td colSpan={7} className="px-5 py-12 text-center">
                      <p
                        ...
                        style={{ fontSize: "0.875rem" }}
                      >
                        No brandAmbassadors match your filters.
                      </p>
                    </td>
                  </tr>
                ) : (
                  paged.map((brandAmbassador) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <Badge
                          variant="secondary"
                          className={getStatusColor(brandAmbassador.status)}
                      ...
                      <td className="px-5 py-3">
                        {brandAmbassador.qualityScore > 0 ? (
                          <div className="flex items-center gap-1.5">
                            <span
                              className={`tabular-nums ${getScoreColor(brandAmbassador.qualityScore)}`}
                              style={{
                                fontSize: "0.875rem",
                                fontWeight: 600,
                              }}
                            ...
                            <TrendIcon trend={brandAmbassador.trend} />
                          </div>
                        ) : (
                          <span
                            className="text-muted-foreground"
          ...
          {filtered.length > 0 && (
            <div className="flex items-center justify-between px-5 py-3 border-t border-border">
              ...
              >
                ...
                {filtered.length} brandAmbassadors
              </span>
