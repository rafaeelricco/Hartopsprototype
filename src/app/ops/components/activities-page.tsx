...
export interface EventRecord {
  ...
  // P1 #4 — Activity-edit lockout when payroll is locked. Mirrors billing-lock.
...
export const MOCK_EVENTS: EventRecord[] = [
  {
    ...
    assignedBrandAmbassadors: [
      ...
      { brandAmbassadorId: "EDU-009", name: "Jasmine Williams", status: "Accepted" },
    ],
  },
  {
    id: "EVT-3002",
  ...
  },
  {
    ...
    assignedBrandAmbassadors: [
      ...
      { brandAmbassadorId: "EDU-006", name: "Marcus Taylor", status: "Pending" },
    ],
  },
  {
    id: "EVT-3005",
  ...
  },
  {
    ...
    assignedBrandAmbassadors: [
      { brandAmbassadorId: "EDU-003", name: "Ashley Chen", status: "Pending" },
    ],
  },
  {
    id: "EVT-3007",
    ...
    capacity: 150,
    description:
      "Interactive workshop for brand ambassadors on integrating digital learning tools into K-12 curriculum.",
    campaignName: "Q1 Brand Awareness",
  ...
  },
  {
    ...
    assignedBrandAmbassadors: [
      { brandAmbassadorId: "EDU-007", name: "Lauren Nguyen", status: "Accepted" },
  ...
  },
  {
    ...
    assignedBrandAmbassadors: [
      { brandAmbassadorId: "EDU-001", name: "Maria Santos", status: "Accepted" },
      { brandAmbassadorId: "EDU-012", name: "Tyler Robinson", status: "Accepted" },
    ],
  },
  {
    id: "EVT-3012",
...
function getAssignmentLabel(brandAmbassadors: AssignedBrandAmbassadorRecord[]) {
  const lead = brandAmbassadors[0];
  if (!lead) return null;
  const first = lead.name.split(" ");
  const shortName = `${first[0]} ${first[1]?.[0] ?? ""}.`;
  const rest = brandAmbassadors.length - 1;
  return rest > 0 ? `${shortName} +${rest}` : shortName;
}

function getAssignmentFilterValue(brandAmbassadors: AssignedBrandAmbassadorRecord[]) {
  if (brandAmbassadors.length === 0) return "unassigned";
  if (brandAmbassadors.some((e) => e.status === "Pending")) return "pending";
  return "assigned";
}
...
export function ActivitiesPage() {
  ...
  const [localAssignments, setLocalAssignments] = useState<
    Record<string, AssignedBrandAmbassadorRecord[]>
  >({});

  // Get effective assignments for an event (local override or mock data)
  const getAssignments = (event: EventRecord) =>
    localAssignments[event.id] ?? event.assignedBrandAmbassadors;

  const handleOpenAssignment = (activityId: string) => {
    const event = MOCK_EVENTS.find((e) => e.id === activityId);
    if (!event) return;
    const current = getAssignments(event);
    setActiveEventId(activityId);
    setDraftSelectedIds(new Set(current.map((e) => e.brandAmbassadorId)));
    setAssignmentSearch("");
  ...
  const handleConfirmAssignments = () => {
    ...
    const newAssignments: AssignedBrandAmbassadorRecord[] = [];
    draftSelectedIds.forEach((id) => {
      const prev = existing.find((e) => e.brandAmbassadorId === id);
      if (prev) {
        newAssignments.push(prev);
      } else {
        const edu = MOCK_BRAND_AMBASSADORS.find((e) => e.id === id);
        if (edu) {
          newAssignments.push({
  ...
  return (
    <div className="p-6 space-y-6 w-full">
      {/* Header */}
      <div>
        <h1 className="text-foreground">Activities</h1>
        <p
      ...
      {/* <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div
          ...
          onClick={() => navigate("/ops/dashboard/draft-activities")}
        >
          <StatCard
            icon={ClipboardCheck}
                ...
                {paged.length === 0 ? (
                  ...
                  paged.map((event) => (
                    <tr
                      key={event.id}
                      onClick={() =>
                        navigate(`/ops/dashboard/activities/${event.id}`)
                      }
                    ...
                    >
                      ...
                      {/* Brand Ambassadors column */}
                      <td className="px-5 py-3">
                        {(() => {
                          const brandAmbassadors = getAssignments(event);
                          ...
                          const hasPending = brandAmbassadors.some(
                            (e) => e.status === "Pending",
                          );
                          return (
                            <div className="flex items-center gap-1.5 min-w-[120px]">
                              ...
                              >
                                {getAssignmentLabel(brandAmbassadors)}
                              </span>
                              {hasPending && (
                                <span
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col p-0 overflow-hidden">
          <DialogHeader className="px-5 pt-5 pb-3 shrink-0 border-b border-border">
            <DialogTitle style={{ fontSize: "1.125rem", fontWeight: 600 }}>
              {(() => {
                ...
                if (!ev) return "Manage Brand Ambassadors";
            ...
            <DialogDescription style={{ fontSize: "0.875rem" }}>
              {(() => {
                ...
                return ev
                  ...
                  : "Select brand ambassadors to assign to this event.";
              })()}
            </DialogDescription>
          ...
          <div className="flex-1 overflow-y-auto p-5 pb-0 bg-muted/20">
            <div className="divide-y divide-border rounded-lg border border-border bg-card overflow-hidden">
              {MOCK_BRAND_AMBASSADORS.filter((e) => e.status === "active")
                ...
                .map((brandAmbassador) => {
                  const isSelected = draftSelectedIds.has(brandAmbassador.id);
                  return (
                    <div
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Checkbox
                          id={`list-edu-${brandAmbassador.id}`}
                          checked={isSelected}
                          className="mt-0.5"
                          onCheckedChange={(checked) => {
                            setDraftSelectedIds((prev) => {
                              const next = new Set(prev);
                              if (checked) next.add(brandAmbassador.id);
                              else next.delete(brandAmbassador.id);
                              return next;
                            });
                          }}
                        />
                        <div className="flex items-center justify-between flex-1 min-w-0">
                          <div className="space-y-0.5 flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <label
                                htmlFor={`list-edu-${brandAmbassador.id}`}
                            ...
                            >
                              ...
                              <span>
                                {brandAmbassador.city}, {brandAmbassador.state}
                              </span>
                              <span>{brandAmbassador.eventsCompleted} events</span>
                            </div>
                            {brandAmbassador.specialties.length > 0 && (
                              <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                                {brandAmbassador.specialties.map((spec) => (
                                  <span
                                    key={spec}
          ...
          <div className="p-5 border-t border-border shrink-0 bg-muted/10 flex items-center justify-between gap-3 rounded-b-lg">
            <span className="text-xs text-muted-foreground">
              {draftSelectedIds.size} brandAmbassador
